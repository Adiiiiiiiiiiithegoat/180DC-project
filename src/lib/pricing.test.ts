...
test("8b. allocation never loses or invents a paise, across many shapes", () => {
  ...
  for (const amounts of shapes) {
    ...
    for (let d = 0; d <= Math.min(total, 200); d++) {
...
test("a discount rule 8 cannot allocate without a negative line is refused, not written", () => {
  // Three 1-paise lines, 2 paise off: floors are 0,0,0, so rule 8 sends the
  // whole 2-paise remainder to the largest line, which is only worth 1. That
  // would be a line with -1 revenue. priceBasket refuses instead.
  assert.throws(
    () =>
      priceBasket(
        [
          { productId: MUG, quantity: 1, unitPrice: 1 },
          { productId: PEN, quantity: 1, unitPrice: 1 },
          { productId: "prod-jar", quantity: 1, unitPrice: 1 },
        ],
        [],
        NOW,
        2,
